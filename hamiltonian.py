...
def set_tpd_tpp(Norb,tpd,tpp,pds,pdp,pps,ppp):
    ...
    if pam.Norb==7:
        ...
        c = np.sqrt(3)/2.0
        tpd_nn_hop_fac = {('d3z2r2','L','px1'): -pds/2.0,\  #平常做不到9个轨道 不用看
    ...
    if pam.Norb==7:
        ...
        tpp_nn_hop_fac = {('UR','px1','px2'):  0.5*(ppp-pps),\ #跳跃方向
...
def create_interaction_matrix_ALL_syms(VS,d_double,p_double,S_val, Sz_val, AorB_sym, A, Upp):
    ...
    for sym in channels:
        ...
        for i in d_double:
            ...
            if o12 not in sym_orbs or S12!=Stot or Sz12 not in Sz_set: #自旋要满足对称性
                continue
